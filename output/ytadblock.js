...
(function initWKWebViewUserscriptCompat() {
  if (typeof window === "undefined") return;
  if (window.__vBlockTubeWKCompatInitialized) return;
  window.__vBlockTubeWKCompatInitialized = true;

  const storagePrefix = "__vblocktube_gm__";
  ...
  if (typeof window.GM_getValue !== "function") {
    window.GM_getValue = function (key, defaultValue) {
      try {
        const raw = localStorage.getItem(storagePrefix + String(key));
  ...
  if (typeof window.GM_setValue !== "function") {
    window.GM_setValue = function (key, value) {
      try {
        localStorage.setItem(storagePrefix + String(key), JSON.stringify(value));
      } catch (e) {}
    };
  }
  ...
  if (typeof window.GM_deleteValue !== "function") {
    window.GM_deleteValue = function (key) {
      try {
        localStorage.removeItem(storagePrefix + String(key));
      } catch (e) {}
    };
  }
  ...
  if (typeof window.GM_addValueChangeListener !== "function") {
    window.GM_addValueChangeListener = function () {
      return -1;
    };
  }

  if (typeof window.GM_removeValueChangeListener !== "function") {
  ...
  if (typeof window.GM_info === "undefined") {
    window.GM_info = {
      scriptHandler: "WKWebView",
      script: { name: "vBlockTube" }
    };
  }
})();
